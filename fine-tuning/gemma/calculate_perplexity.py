    ...
    def _compute(
        ...
        if chat_template:
            ...
            for pred in predictions:
                chat_message = [{"role": "assistant", "content": pred}]
                chat_pred = tokenizer.apply_chat_template(
                    chat_message, tokenize=False, add_generation_prompt=True
                )
                chat_predictions.append(chat_pred)
            predictions = chat_predictions
...
VOLUME_CONFIG: Mapping[Union[str, PurePosixPath], modal.Volume] = {
...
def launch(
    ...
    if not input_texts:
        print("Warning: No input texts provided.")
        return

    pplx = perplexity._compute(
